...
# Check dotnet core 2.2 dependencies for Linux
if [[ (`uname` == "Linux") ]]
then
    ...
    if [ $? -eq 0 ]; then
        echo "Dependencies is missing for Dotnet Core 2.2"
        echo "Execute ./bin/installdependencies.sh to install any missing Dotnet Core 2.2 dependencies."
        exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo "Libicu's dependencies is missing for Dotnet Core 2.2"
        echo "Execute ./bin/installdependencies.sh to install any missing Dotnet Core 2.2 dependencies."
        exit 1
    fi
fi
